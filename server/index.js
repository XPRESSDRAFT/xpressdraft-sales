...
// Calculate commission for a given weekly sales total and role
// Tier rate applies to the ENTIRE weekly total, not segments
function calcCommission(total, role) {
  if (role === 'leader') {
    if (total <= 8000) return total * 0.03;
    if (total <= 15000) return total * 0.04;
    if (total <= 25000) return total * 0.05;
    return total * 0.06;
  } else {
    if (total <= 15000) return total * 0.02;
    if (total <= 25000) return total * 0.03;
    return total * 0.04;
  }
}
