...
function resetForm(){state.checks={};state.editingId=null;setExp('new');apply({});$('#cName').value='';$('#cAddr').value='';$('#cContact').value='';$('#cDate').value=new Date().toISOString().slice(0,10);renderChecklist();updateProgress();$('#editingName').textContent='New client (unsaved)';}
function saveCurrent(){const name=$('#cName').value.trim();if(!name){toast('Enter a client name first');$('#cName').focus();return;}const rec={id:state.editingId||uid(),name,addr:$('#cAddr').value,contact:$('#cContact').value,date:$('#cDate').value,exp:state.exp,fields:gather(),checks:{...state.checks},updated:Date.now()};saveRecord(rec,()=>{state.editingId=rec.id;$('#editingName').textContent=name;toast('Saved "'+name+'"');renderSaved();});}
function openClient(id){load(list=>{const c=list.find(x=>x.id===id);if(!c)return;state.editingId=c.id;state.checks={...(c.checks||{})};setExp(c.exp||'new');$('#cName').value=c.name;$('#cAddr').value=c.addr||'';$('#cContact').value=c.contact||'';$('#cDate').value=c.date||'';apply(c.fields||{});renderChecklist();updateProgress();$('#editingName').textContent=c.name;window.scrollTo({top:0,behavior:'smooth'});toast('Opened "'+c.name+'"');});}
